...
export function NewVoirDireSetDialog({
  caseId,
  onClose,
}: {
  caseId: string;
  ...
  const { data: nextNumber } = trpc.voirDire.getNextSetNumber.useQuery({
    caseId,
    servingParty,
  });
  ...
  const create = trpc.voirDire.createSet.useMutation({
    onSuccess: ({ id }) => {
      toast.success("Voir dire set created");
      utils.voirDire.listForCase.invalidate({ caseId });
      router.push(`/cases/${caseId}/trial-prep/voir-dire/${id}`);
    },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-md border border-zinc-700 bg-zinc-950 p-6 text-zinc-100">
        <h2 className="text-lg font-semibold">New Voir Dire Set</h2>
        ...
        <div className="mt-4 space-y-4">
          ...
          <label className="block text-sm">
            ...
            <input
              ...
              placeholder="Plaintiff's Proposed Voir Dire Questions"
            />
          </label>
        </div>
