...
export function NewWitnessListDialog({
  caseId,
  onClose,
}: {
  caseId: string;
  ...
  const { data: nextNumber } = trpc.witnessLists.getNextListNumber.useQuery({
    caseId,
    servingParty,
  });
  ...
  const create = trpc.witnessLists.createList.useMutation({
    onSuccess: ({ id }) => {
      toast.success("Witness list created");
      utils.witnessLists.listForCase.invalidate({ caseId });
      router.push(`/cases/${caseId}/trial-prep/witness-lists/${id}`);
    },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-md border border-zinc-700 bg-zinc-950 p-6 text-zinc-100">
        <h2 className="text-lg font-semibold">New Witness List</h2>
        ...
        <div className="mt-4 space-y-4">
          ...
          <label className="block text-sm">
            ...
            <input
              ...
              placeholder="Plaintiff's Trial Witness List"
            />
          </label>
        </div>
