...
const ORDINAL: Record<number, string> = {
  1: "Proposed",
  2: "First Amended Proposed",
  3: "Second Amended Proposed",
  4: "Third Amended Proposed",
  5: "Fourth Amended Proposed",
};

function defaultTitle(party: Party, n: number): string {
  const partyLabel = party === "plaintiff" ? "Plaintiff" : "Defendant";
  const adj = ORDINAL[n] ?? `${n}th Amended Proposed`;
  return `${partyLabel}'s ${adj} Jury Instructions`;
}

export function NewJuryInstructionSetDialog({
  caseId,
  onClose,
}: {
  caseId: string;
  ...
  const { data: nextNumber } = trpc.juryInstructions.getNextSetNumber.useQuery({
    caseId,
    servingParty,
  });
  ...
  const create = trpc.juryInstructions.createSet.useMutation({
    onSuccess: ({ id }) => {
      toast.success("Jury instruction set created");
      utils.juryInstructions.listForCase.invalidate({ caseId });
      router.push(`/cases/${caseId}/trial-prep/jury-instructions/${id}`);
    },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-md border border-zinc-700 bg-zinc-950 p-6 text-zinc-100">
        <h2 className="text-lg font-semibold">New Jury Instruction Set</h2>
        ...
        <div className="mt-4 space-y-4">
          ...
          <label className="block text-sm">
            ...
            <input
              ...
              placeholder="Plaintiff's Proposed Jury Instructions"
            />
          </label>
        </div>
