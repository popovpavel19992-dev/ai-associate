...
export interface InterrogatoriesPdfProps {
...
const DEFINITIONS: Array<{ heading: string; body: string }> = [
  ...
  {
    heading: "2. ",
    body: "\"Identify,\" when used in reference to a natural person, means to state the person's full name, present or last known business and home address, telephone number, employer, and job title; when used in reference to a document, means to state its date, author(s), recipient(s), type (e.g., letter, email, memorandum), subject matter, and present custodian.",
  },
  {
    heading: "3. ",
    body: "\"Document\" is used in its broadest sense consistent with Federal Rule of Civil Procedure 34 and includes, without limitation, all written, printed, recorded, electronic, or graphic matter, however produced or reproduced, including emails, text messages, and electronically stored information.",
  },
  {
    heading: "4. ",
    body: "These interrogatories are continuing in nature. If You become aware of additional information responsive to any interrogatory after Your initial response, You are required to supplement Your answers in accordance with Federal Rule of Civil Procedure 26(e).",
  },
  {
    heading: "5. ",
    body: "If You withhold any information on the basis of privilege or work-product protection, You must produce a privilege log identifying each item withheld, the basis for the assertion, and sufficient information to permit an assessment of the claim, as required by Federal Rule of Civil Procedure 26(b)(5).",
  },
];

export function InterrogatoriesPdf(props: InterrogatoriesPdfProps): React.ReactElement {
  ...
  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        <Text style={styles.paragraph}>
          {serving}, by and through undersigned counsel, hereby propounds the
          following interrogatories upon {opposing} pursuant to Federal Rule of
          Civil Procedure 33. You are required to answer each interrogatory under
          oath, in writing, within thirty (30) days of service hereof.
        </Text>
        ...
        <Text style={styles.heading}>INTERROGATORIES</Text>
        {request.questions.map((q) => (
          <View key={q.number} style={styles.question} wrap={false}>
            <Text style={styles.questionHeader}>INTERROGATORY NO. {q.number}:</Text>
            <Text>{q.text}</Text>
