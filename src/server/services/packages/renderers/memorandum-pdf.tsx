...
// Memorandum of Law renderer (2.4.3b).
//
// Mirrors motion-pdf layout (same fonts, margins, caption, signature) but
// replaces the document title with "MEMORANDUM OF LAW IN SUPPORT OF [title]"
// and is the document that actually carries the facts/argument/conclusion
// sections when a motion is rendered as a split filing.
...
export function MemorandumPdf({
  ...
  const memoTitle = `MEMORANDUM OF LAW IN SUPPORT OF ${caption.documentTitle.toUpperCase()}`;
  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        <Text style={[styles.bold, styles.center, { fontSize: 14, marginBottom: 16 }]}>{memoTitle}</Text>
        {skeleton.sections
          .filter((s) => s.type === "ai")
          .map((s) => {
