...
describe("OutlookCalendarProvider mapping", () => {
  it("maps timed event to Outlook/Graph format", () => {
    ...
    expect((result.start as any).dateTime).toBe("2026-04-22T14:00:00.000Z");
    expect((result.start as any).timeZone).toBe("UTC");
    expect((result.end as any).dateTime).toBe("2026-04-22T15:00:00.000Z");
    expect((result.location as any).displayName).toBe("Office");
    expect((result.body as any).content).toContain("Discuss settlement");
    expect((result.body as any).content).toContain("Managed by ClearTerms");
  ...
  it("maps all-day event to Outlook format", () => {
    ...
    expect((result.start as any).dateTime).toBe("2026-04-22T00:00:00.000Z");
  });
});
